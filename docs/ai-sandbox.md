...
| Profile bridge | `ai0` | `10.254.77.1/24`, IPv4 NAT, IPv6 off; local/cloud only |
...
`10.254.77.1`, which sits inside the `10.0.0.0/8` drop. The allow rules for port
53 and 67 to `10.254.77.1/32` must come first, or the container has no name
...
scripts/ai-sandbox.sh bash -c '
  ...
  timeout 5 bash -c "exec 3<>/dev/tcp/10.254.77.1/22"  || echo "host blocked"'
```
