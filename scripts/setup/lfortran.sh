...
cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$INFRA/.venv
make -j$(nproc)
make install
popd
...
cmake .. -D CMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$INFRA/.venv
make -j$(nproc)
make install
popd
...
cmake .. -DCMAKE_BUILD_TYPE=Debug -DWITH_LLVM=yes -DCMAKE_INSTALL_PREFIX=$INFRA/.venv -DWITH_XEUS=yes
make -j$(nproc)
make install
popd
